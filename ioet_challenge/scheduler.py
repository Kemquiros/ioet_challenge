...
def select_slot( h_init, m_init):
    '''Select one slot according to the hour:minutes'''
    slot = None
    if (h_init == 0 and m_init >= 1) or \
            (h_init > 0 and m_init >= 0 and h_init<=8 and m_init<=59) or \
            (h_init == 9 and m_init == 0):
        slot = 1
    elif (h_init == 9 and m_init >= 1) or \
            (h_init > 9 and m_init >= 0 and h_init<=17 and m_init<=59) or \
            (h_init == 18 and m_init == 0):
        slot = 2
    elif (h_init == 18 and m_init >= 1) or \
            (h_init > 18 and m_init >= 0 and h_init<=23 and m_init<=59) or \
            (h_init == 0 and m_init == 0):
        slot = 3
    return slot

def calc_salary_same_slot(h_init, m_init, h_finish, m_finish):
...
class AcmeScheduler:
    '''This class calculates the salary of each worker according to business logic'''
    ...
    def get_pay_rate(self,day,time_slot):
        '''Set the hour rate according to the time and day'''
    ...
    def calc_salary_slot(self, d, h_init, m_init, h_finish, m_finish):
        ...
        salary = 0
        slot_init = select_slot(h_init, m_init)
        slot_finish = select_slot(h_finish, m_finish)

        if slot_init == 1:
            if slot_finish == 1:
                salary += calc_salary_same_slot(h_init, m_init, h_finish, m_finish)
    ...
    def calc_salary(self, worker):
        ...
        for day in weekdays:
            for slot in worker.work[day]:
                ...
                salary_slot = self.calc_salary_slot(
                    day,
                    int(hour_init),
                    int(minutes_init),
                    int(hour_finish),
